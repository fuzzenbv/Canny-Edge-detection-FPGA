...
void gradient_decomposition(GRAY_IMAGE_16S& gx, GRAY_IMAGE_16S& gy, GRAY_IMAGE_16& gd) {
	...
	// usado para proporcionar información adicional para superar las dependencias de los bucles y permitir
				// la segmentación (o una segmentación de menor nivel) NECESARIO PARA ESTE CASO
	short element_gx;
	short element_gy;
...
void nonmax_suppression(GRAY_IMAGE_16& gd, GRAY_IMAGE_16& dst) {
	...
	hls::LineBuffer<2, 1920, unsigned short> lbuffer;
	...
	unsigned short tmp0;
	unsigned short tmp1;
	for( HLS_SIZE_T i = 0; i < gd.rows+1; i++ ) {
	#pragma HLS loop_tripcount min=0 max=1080
		for( HLS_SIZE_T j = 0; j < gd.cols+1; j++ ) {
		#pragma HLS loop_tripcount min=0 max=1920
		#pragma HLS LOOP_FLATTEN OFF
		#pragma HLS DEPENDENCE array inter false
		#pragma HLS PIPELINE
		  ...
		  if( j < gd.cols ) {
			tmp1 = lbuffer.getval(1, j);
			tmp0 = lbuffer.getval(0, j);
			// shift values up 1 row
			lbuffer.val[1][j] = tmp0;
		  }
		  // Insert a new pixel into the bottom right of the line buffer if it is present
		  if( j < gd.cols && i < gd.rows ){
			lbuffer.insert_bottom( element_gd, j );
		  }
		  ...
		  if( i <= 1 || j <= 1 || i > gd.rows-1 || j > gd.cols-1 ) {
			out_pixel_val = 0;
		  }
		  else {
			// Checking in the X axis (0º)
			if ( current_dir == 0/*0*/ ) {
			  ga = win.getval( 1, 0 )>>2;
			  gb = win.getval( 1, 2 )>>2;
			}
			// Checking in diagonal 135º
			else if ( current_dir == 3/*1*/ ){
			  ga = win.getval( 2, 0 )>>2;
			  gb = win.getval( 0, 2 )>>2;
			}
			// Checking in the Y axis (90 or -90)
			else if ( current_dir == 2/*2*/ ){
			  ga = win.getval( 0, 1 )>>2;
			  gb = win.getval( 2, 1 )>>2;
			}
			// Checking in diagonal 45º
			else { // ( current_dir == 1 )
			  ga = win.getval( 2, 2 )>>2;
			  gb = win.getval( 0, 0 )>>2;
			}
...
#pragma HLS dataflow
	const int col_packets = cols/4; // /4
	const int packets = col_packets*rows;
	const int pixel_cnt = rows*cols;

	/**
	 * II=4 y no 1 debido a la dependencia que tienen los 4 pixeles de dat, deben finalizar los 4 pixeles para empezarla siguiente instruccion
	 * y cargar la siguiente imagen
	 *
	 * Cada iteracion de 32bits en 32bits
	 */
	...
	//hls::GaussianBlur<5,5>( src_bw, src_bw, 1.4, 1.4 );
